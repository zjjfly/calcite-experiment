...
public class MySqlSqlWindowStartEnd implements WindowAggregationRule.SqlWindowStartEnd {
  ...
  public RexNode windowStart(RelDataTypeFactory typeFactory, RexNode timeCol, RexLiteral interval) {
    ...
    if (typeName != SqlTypeName.INTERVAL_MONTH && typeName != SqlTypeName.INTERVAL_YEAR) {
      itl =
          rexBuilder.makeLiteral(intervalValue.divide(BigDecimal.valueOf(1000), 2, RoundingMode.FLOOR).intValue(),
              itl.getType());
    ...
    RexNode intervalCount =
        rexBuilder.makeCall(SqlStdOperatorTable.MULTIPLY, rexBuilder.makeCall(SqlStdOperatorTable.FLOOR,
            rexBuilder.makeCall(SqlStdOperatorTable.DIVIDE, Lists.newArrayList(timeDiff, itl))), itl);
  ...
  /**
   * 构造类似下面的表达式 timestampadd(YEAR, floor(timestampdiff(YEAR, '1970-01-01', timeCol) / 3 + 1) * 3 , "1970-01-01")
   */
  @Override
